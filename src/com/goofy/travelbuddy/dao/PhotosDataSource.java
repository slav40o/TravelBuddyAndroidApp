...
public class PhotosDataSource {
	...
	private String[] allColumns = { 
			...
			PhotosSQLiteHelper.COLUMN_NAME,
			PhotosSQLiteHelper.COLUMN_IMAGE,
    ...
    public Photo createPhoto(Photo photo){
        ...
        Cursor cursor = database.query(PhotosSQLiteHelper.TABLE_PHOTOS,
                allColumns, PhotosSQLiteHelper.COLUMN_ID + " = " + insertId, null,
                null, null, null);
        cursor.moveToFirst();
        Photo newPhoto = cursorToComment(cursor);
        cursor.close();	
        return newPhoto;
    }
    
    private Photo cursorToComment(Cursor cursor) {
		 Photo place = new Photo(
				 cursor.getInt(0), 
				 cursor.getString(1),
				 cursor.getBlob(2),
				 cursor.getString(3), 
